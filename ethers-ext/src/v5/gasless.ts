 ...
 */
export function getAmountRepay(approveRequired: boolean, gasPrice: number | string | BigNumber = 25): string {
    const gasPriceBN = 
        typeof gasPrice === 'number' 
            ? BigNumber.from(Math.floor(gasPrice * 1e9))
...
export async function getApproveRawTx(
    ...
    try {
        ...
        // Convert gasPrice to appropriate format
        ...
        const tx = {
            ...
            nonce: nonce,
            gasLimit: 100000,
            gasPrice: gasPriceBN, // Use BigNumber directly
            data: approveData,
            value: Zero,
            chainId: chainId,
        };
